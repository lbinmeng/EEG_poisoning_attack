...
parser.add_argument('--data', type=str, default='P300', help='name of data, ERN or P300')
...
for r in range(repeat):
    ...
    results_dir = os.path.join(save_dir, data_name, 'xDAWN', 'run{}'.format(r))
...
if partial:
    np.savez(os.path.join(save_dir, data_name, 'xDAWN') + '/{}.npz'.format(partial), raccs=raccs,
             rbcas=rbcas,
             rasrs=rasrs)
else:
    if not baseline:
        np.savez(os.path.join(save_dir, data_name, 'xDAWN') + '/npp_{}_{}_{}.npz'.format(npp_params[0], npp_params[1],
                                                                                         npp_params[2]), raccs=raccs,
                 rbcas=rbcas,
                 rpoison_rates=rasrs)
    else:
        np.savez(
            os.path.join(save_dir, data_name, 'xDAWN') + '/baseline_{}_{}_{}.npz'.format(npp_params[0], npp_params[1],
                                                                                         npp_params[2]), raccs=raccs,
            rbcas=rbcas,
            rpoison_rates=rasrs)
